/**
 * MaskedHeading — React Bits (JS + CSS variant), vendored.
 *
 * ONE deliberate deviation from upstream, marked FONT CEILING below: the 200px
 * type clamp is now the `maxFontSize` prop instead of a hard-coded literal. The
 * DEFAULT is unchanged at 200, so out of the box this sizes exactly as upstream
 * does. Everything else is upstream source, unmodified — keep it that way so it
 * stays swappable.
 */
...
const MaskedHeading = ({
  ...
  lineHeight = 1.06,
  textScale = 0.115,
  maxFontSize = 200, // FONT CEILING — upstream value; exposed as a prop
  className = '',
  style,
  ...rest
}) => {
  ...
  const sync = useCallback(() => {
    ...
    root.style.fontSize = `${clamp(root.clientWidth * s.textScale, 20, s.maxFontSize).toFixed(1)}px`;
  ...
  useEffect(() => {
    ...
    /* HARDENED RE-SYNC (deviation from upstream).
     
       The clip glyphs carry hard-coded x/y attributes copied from the measuring
       span's layout. Until sync() re-runs after the webfont lands they hold stale
       coordinates — observed here as x=0 while the spans were already at 18 and
       12, which draws the letters in the wrong place and can push the last one
       out of the box. `fonts.ready` normally covers it, but it resolves against
       the fonts pending at that moment, so a late-arriving face can miss it.
       A load-event pass and one settle tick close that window. */
    const resync = () => sync();
    window.addEventListener('load', resync);
    const settle = setTimeout(resync, 400);
